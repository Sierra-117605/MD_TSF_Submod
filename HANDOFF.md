...
最終更新: 2026-09-21（実機ログのエラーを全件修正・3Dテクスチャ修復 / 再テスト待ち）

---

## 2026-09-21 [Claude Code → 本人] 実機ログのエラー修正完了: 再テスト待ち

- ステータス: 修正実施済み / **HOI4 再起動での確認待ち**
- 2026-09-19 の実機ログ（MD正式版 + 本MOD）で出ていたエラーのうち、本MOD由来のものを全件修正
  - `num_research_slots` → `amount_research_slots`（96箇所）
  - AIテンプレの支援中隊をMD 2.0の名前へ、バニラ時代の不要ロール4件を削除
  - `recovery_rate`（1.19に無いキー）を削除
  - モデル用テクスチャ35枚を PNG → DDS(DXT5)、mesh 23件の参照を付け替え
    → mesh のテクスチャ参照171件すべて解決（それまで18件が欠落）
  - 表示名が英語IDのままだった loc 13キーを追加
- 静的チェックはすべて通過：
  装備スロットの継承1834件・装備参照・技術参照・locキー衝突 いずれも問題なし
- **残っているのはテックツリーのグリッド重複14件のみ**（表示位置が不定になるだけ。
  直すにはどの技術をどのツリーに置くかの判断が要るので保留）
- お願い: ランチャーでMD**正式版**に切り替えて新規ゲーム → `logs/error.log` を確認
...
### 反映まで完了済み（2026-09-19）
- 不要ファイル3件を削除（スサノオ大隊の二重定義の原因だった .tmp 残骸を含む）
- OneDrive側（HOI4が読む本体）へ868ファイルを反映。dev側と完全一致、クラウド専用ファイルは0件
- 登録ファイル `mod/MD_TSF_Submod.mod` も 1.19 + 正式版依存に更新
- GitHubへpush済み（ブランチ `feat/1.19-compat`）

### 本人にお願いしたいこと
1. ランチャーのプレイセットで、MD を **正式版**（"Millennium Dawn: A Modern Day Mod"）に切り替える
   （2026-09-19時点の設定は MD **Beta** が有効。本MODの依存先は正式版に変更済み）
2. 本サブMODを有効化して新規ゲームを開始
3. `logs/error.log` に MVLV / TSF 関連のエラーが出ていないか確認
4. 研究画面の戦術機タブが正しい位置に出るか目視確認
