...
## 🟢 3Dモデルのテクスチャが読めない問題の解決（2026-09-21）

### 原因
HOI4の3Dモデルは **.dds しか読めない**。`gfx/models/` のテクスチャ35枚が
中身もファイル名も PNG のままで、`.mesh`（モデル本体）もそれを参照していた。
結果 `Failed to find texture 'XXX_color.png'` が出て、戦術機がテクスチャ無しで表示されていた。

### 直し方
1. PNG を **DXT5圧縮のDDS** に変換（Pillow で `save(..., "DDS", pixel_format="DXT5")`）。
   既存の正常なテクスチャもすべて DXT5 だったので形式を合わせた
2. `.mesh` はバイナリだが、参照しているファイル名がそのまま文字列として入っている。
   `".png"` → `".dds"` は**同じ4文字**なので、バイト数を変えずに置換できる（23ファイル）
3. 参照名と実ファイル名がズレていたもの（`GEU_F4_color` と `GEU_Tsf_F4_color`）は
   正しい名前でDDSを作って補完
4. 参照先が存在しなかった2件は、同系統の既存テクスチャを複製して補完

### 注意
- `JAP_Tsf_00R_color .png` のように**ファイル名に空白**が混ざっているものがある。
  mesh 側もその空白ごと参照しているので、**空白を消してはいけない**（消すと参照が切れる）
- Pillow の DDS 保存は **ミップマップを作らない**。遠景で多少ジャギーが出る可能性はあるが表示はされる
- 元の .png は消さずに残してある（参照はすべて .dds に向いている）

---

## 🟡 残っている既知の不具合（2026-09-21 時点・未対応）
1. **テックツリーのグリッド重複**（`Found multiple potential grid boxes` 14件）
   - `MVLV_research_armor_composite` など数件が、軽装甲ツリーと重装甲ツリーの
     両方に属していてHOI4が置き場所を決められない
   - 影響：研究画面での表示位置が不定
2. **`sov_equipment_Su47`**
   - 装備定義はあるが、どの技術からも解放されていない（死んだ定義）。表示名だけ付けた
   - 使うなら技術側に `enable_equipments` を足す必要がある

（`recovery_rate` は 1.19 で無効なキーと確定したため 2026-09-21 に削除済み）
