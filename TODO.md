...
### 2026-09-19 追加で実施
- [x] **6/14の修正をdev側へ復旧**（OneDriveにしか無かった8ファイル。詳細は KNOWLEDGE.md の事故記録）
- [x] **無効なトリガ `num_research_slots` → `amount_research_slots`**（96箇所）
- [x] **AIテンプレの支援中隊をMD 2.0の名前へ**、バニラ時代の不要ロール4件を削除
- [x] **表示名が英語IDのまま出ていた loc 12キーを追加**

### 要確認
- [x] **`recovery_rate` を削除**（error.log で `Unexpected token` と確定。1.19に存在しないキー）
...
- [x] **3Dモデルのテクスチャ欠落を解消**（PNG35枚をDXT5のDDSに変換、mesh23件の参照を付け替え、名前ズレ・不足4件を補完 → 参照欠落0件）
- [ ] **テックツリーのグリッド重複14件**（軽/重装甲ツリーの両方に属する技術がある）
